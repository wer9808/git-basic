...
git merge를 수행하면 두 브랜치 변경사항이 합쳐진 새로운 커밋이 생성되고, 현재 브랜치는 해당 커밋을 가리키게 된다. target 브랜치의 커밋 히스토리가 base 브랜치에 추가된다.

## git rebase

git rebase 명령어를 통해서도 다른 브랜치의 변경사항을 현재 브랜치에 반영할 수 있다. merge 명령어와 다른 점은 merge 커밋을 생성하지 않는다는 점이다.
